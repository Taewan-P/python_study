...
class Hogaeng:
	...
	def level_button(self):
		...
		self.level=IntVar()
		self.level.set(None)
		Radiobutton(self, text='Hard', variable=self.level, value=40, command=self.level.set(40)).grid(row=1, column=12, sticky=W)
		Radiobutton(self, text='Normal', variable=self.level, value=34, command=self.level.set(34)).grid(row=2, column=12, sticky=W)
		Radiobutton(self, text='Easy', variable=self.level, value=28, command=self.level.set(28)).grid(row=3, column=12, sticky=W)

		#이게 왜 레벨_버튼에 있는거지?
		Button(self, text='GO', command=sw.Start).grid(row=2, column=13, columnspan=3)  # 나중에 크기 키우기


		return int(self.level.get())
...
class SudokControllor(Frame):
	def __init__(self, master):
		...
		self.no_of_holes = Hogaeng.level_button(self)
